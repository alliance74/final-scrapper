"""
Test saving existing scraped data to database
Uses the JSON files that were already scraped
"""
from database import SessionLocal, Event, init_db
from data_transformer import DataTransformer
import json
import os

def load_existing_json():
    """Load existing JSON files"""
    events_by_source = {}
    
    json_files = {
        'more_events': 'scraped_data/more_events_optimized.json',
        'pigolampides': 'scraped_data/pigolampides_events.json',
        'visitgreece': 'scraped_data/visitgreece_all_events.json',
        'culture_gov': 'scraped_data/culture_gov_final_events.json'
    }
    
    for source, filepath in json_files.items():
        if os.path.exists(filepath):
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    # Limit to 5 events per source for testing
                    events_by_source[source] = data[:5]
                    print(f"✓ Loaded {len(events_by_source[source])} events from {source}")
            except Exception as e:
                print(f"✗ Error loading {source}: {e}")
                events_by_source[source] = []
        else:
            print(f"⚠ File not found: {filepath}")
            events_by_source[source] = []
    
    return events_by_source

def test_save_existing():
    """Test transforming and saving existing data"""
    
    print("="*60)
    print("Test: Save Existing Scraped Data")
    print("="*60)
    
    # Initialize database
    print("\n[1/4] Initializing database...")
    init_db()
    db = SessionLocal()
    print("      ✓ Database initialized")
    
    # Load existing JSON
    print("\n[2/4] Loading existing JSON files...")
    events_by_source = load_existing_json()
    
    total_loaded = sum(len(events) for events in events_by_source.values())
    print(f"\n      Total events loaded: {total_loaded}")
    
    if total_loaded == 0:
        print("\n      ✗ No events found in JSON files!")
        print("      Run scrapers first to generate data.")
        return False
    
    # Transform data
    print("\n[3/4] Transforming data...")
    transformer = DataTransformer()
    standardized = transformer.transform_all_events(events_by_source)
    print(f"      ✓ Transformed {len(standardized)} events")
    
    # Save to database
    print("\n[4/4] Saving to database...")
    saved_count = 0
    
    for event_data in standardized:
        try:
            # Check if exists
            url = event_data.get('url') or event_data.get('eventUrl')
            if url:
                existing = db.query(Event).filter(Event.url == url).first()
                if existing:
                    print(f"      ⚠ Skipping duplicate: {event_data.get('title', '')[:40]}")
                    continue
            
            # Create event
            event = Event(
                title=event_data.get('title', 'Untitled'),
                description=event_data.get('description'),
                date=event_data.get('date'),
                location=event_data.get('location') or event_data.get('venue'),
                category=event_data.get('category'),
                price=str(event_data.get('price', 0)),
                url=url,
                source=event_data.get('source', 'Unknown'),
                images=[event_data.get('image')] if event_data.get('image') else [],
                contact=None,
                content={'region': event_data.get('region'), 'venue': event_data.get('venue')},
                full_text=None
            )
            
            db.add(event)
            db.commit()
            saved_count += 1
            print(f"      ✓ Saved: {event.title[:50]}")
            
        except Exception as e:
            print(f"      ✗ Error saving: {e}")
            db.rollback()
            continue
    
    # Verify
    print("\n" + "="*60)
    print("Results:")
    print("="*60)
    print(f"Events transformed: {len(standardized)}")
    print(f"Events saved: {saved_count}")
    
    total_in_db = db.query(Event).count()
    print(f"Total in database: {total_in_db}")
    
    if saved_count > 0:
        print("\n✅ Success! Events saved to database.")
        
        # Show samples
        print("\nSample events:")
        samples = db.query(Event).limit(3).all()
        for i, event in enumerate(samples, 1):
            print(f"\n[{i}] {event.title[:60]}")
            print(f"    Date: {event.date}")
            print(f"    Category: {event.category}")
            print(f"    Source: {event.source}")
            print(f"    Price: {event.price}")
    else:
        print("\n⚠ No new events saved (may already exist in database)")
    
    db.close()
    return saved_count > 0

if __name__ == "__main__":
    test_save_existing()
